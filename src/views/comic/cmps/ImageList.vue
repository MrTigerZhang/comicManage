...
export default class ComicContentManager extends Vue {
  ...
  private async dropzoneSuccess(file: File, response: any) {
    ...
    await addContent({ chapterId: this.chapterId, imgSrc: response.data.key });
    await this.queryContent();
    ...
    this.sortable = Sortable.create(el, {
      ...
      onEnd: async (evt) => {
        if (
          ...
          //获取oldId
          const oldId = this.comicList[evt.oldIndex];
          const anotherId = this.comicList[evt.newIndex];
          // 调用您的 API 更新排序，例如：
