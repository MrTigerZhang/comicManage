  ...
  <div v-if="isEditMode" class="components-container">
    ...
    <template>
        ...
        <el-row :gutter="20">
          <el-col :span="6">
            <el-form-item label="章节序号">
              <el-input
                v-model.number="searchForm.chapterNo"
                type="number"
                :min="1"
              ></el-input>
            </el-form-item>
      ...
      >
        ...
        <el-table-column label="缩略图" prop="thumbnailUrl">
          <template slot-scope="{ row }">
            <el-image
              :src="row.thumbnailUrl2"
              :preview-src-list="[row.thumbnailUrl2]"
              :fit="'contain'"
              style="width: 100px; height: 100px"
            ></el-image>
          </template>
        </el-table-column>
        <el-table-column label="总页数" prop="pageCount"></el-table-column>
...
export default class ChapterManager extends Vue {
  ...
  async searchChapters() {
    ...
    try {
      const { dataList, total } = (
        await queryChapters({
          data: {
            comicId: this.comicId,
            name: this.searchForm.name,
            chapterNo: this.searchForm.chapterNo,
            createDateStart: this.searchForm.createdAtFrom
              ? this.searchForm.createdAtFrom[0]
              : "",
            createDateEnd: this.searchForm.createdAtFrom
              ? this.searchForm.createdAtFrom[1]
              : "",
            updateDateStart: this.searchForm.updatedAtFrom
              ? this.searchForm.updatedAtFrom[0]
              : "",
            updateDateEnd: this.searchForm.updatedAtFrom
              ? this.searchForm.updatedAtFrom[1]
              : "",
